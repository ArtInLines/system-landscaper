...
import SingleLayerTreeMapView from './Views/SingleLayerTreeMapView';
import SystemNode from './Graphs/SystemNode';

interface GetDataListFieldFunction {
	(label: string, val: any, values: any[], onChange: (x: any) => void): HTMLDivElement;
	idCounter?: number;
}

const main = (e: Event | null = null) => {
	const graphContainer = document.getElementById('graph-root') as HTMLDivElement;
	const newNodeBtn = document.getElementById('btn-new') as HTMLButtonElement;
	const viewSelect = document.getElementById('select-view') as HTMLSelectElement;
	...
	function getDefaultNodeData(node: SystemNode | null = null) {
		return {
			name: node?.name || '',
	...
	function getInputField(label: string = 'Label', value: any = null, placeholder: string = 'Placeholder', onChange: (x: any) => void = () => {}) {
	...
	function getSelectField(label: string, values: any[], isSelected: (el: HTMLOptionElement) => boolean, onChange: (x: any) => void) {
		const container = document.createElement('div');
	...
	const getDataListField: GetDataListFieldFunction = function (label: string, val: any, values: any[], onChange: (x: any) => void) {
	...
	function getNodeEditorHTML(submit: null | ((data: any, node: null | SystemNode) => void) = null, node: null | SystemNode = null) {
		...
		if (submit) submitBtn.addEventListener('click', (e) => submit(data, node));
	...
	function updateNodeHTML(node: SystemNode) {
		clearSideBar();
		sideBar.appendChild(getNodeEditorHTML(updateNode, node));
	}

	function clearSideBar(data: any = null) {
		sideBar.childNodes.forEach((c) => c.remove());
	}

	function submitNewNode(data: any, node: null | SystemNode) {
	...
	function updateNode(data: any, node: null | SystemNode) {
		if (node) {
	...
	function addOption(selectEl: HTMLSelectElement, text: string, value: any, isSelected: (el: HTMLOptionElement) => boolean) {
	...
	function updateViewOptions() {
		...
		const isSelected = (opt: HTMLOptionElement) => selectedOptIds.includes(opt.value);
